...
def main():
    ...
    if not xml_files:
        print(f"[LOG] No XML files found in {xbrl_xml_folder}")
        return

    print(f"[LOG] Found {len(xml_files)} XML file(s) in {xbrl_xml_folder}")
    for xml_file in xml_files:
        ...
        print(f"[LOG] Processing {xml_file} ...")
        try:
            ...
            print(f"[LOG] Saved JSON to {json_path}")
        except Exception as e:
            print(f"[ERROR] Failed to process {xml_file}: {e}")

if __name__ == "__main__":
    print("[LOG] Starting XBRL XML to JSON batch conversion script...")
    main()
