...
def check_pdf_xbrl(zip_path, temp_dir):
    ...
    try:
        ...
        if not pdf_files:
            print(f"No PDF files found in {zip_name}")
            return
        
        # Ensure No_XBRL directory exists
        no_xbrl_dir = "No_XBRL"
        ...
        for pdf_file in pdf_files:
            ...
            if "XBRL" in pdf_name:
                print(f"File: {pdf_name} - (XBRL)")
            else:
                print(f"File: {pdf_name} - (No-XBRL) -> Moving to {no_xbrl_dir}")
